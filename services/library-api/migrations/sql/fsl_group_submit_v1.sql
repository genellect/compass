...
DECLARE
    v_access_strategy text := COALESCE(p_request->>'access_strategy', 'legacy_individual');
...
BEGIN
    ...
    IF v_access_strategy NOT IN ('legacy_individual', 'group_membership')
       OR (p_request ? 'access_strategy' AND pg_catalog.jsonb_typeof(p_request->'access_strategy') <> 'string')
       OR p_request->>'contract_version' <> 'v1'
    ...
    IF v_member_id IS NOT NULL AND v_final_status = 'approved' THEN
        ...
        v_operation_type := CASE WHEN v_access_strategy = 'group_membership'
            THEN 'group_membership_add' ELSE 'drive_grant' END;
        v_operation_key := v_operation_type || ':' || v_member_id::text
            || ':future-strategy-library-primary-v1';
        IF NOT EXISTS(
            SELECT 1
