...
function requireSafeStage(root, stage) {
  const allowedRoot = path.join(root, "outputs", "library-ui-review");
  ...
  if (
    ...
    throw new Error(
      "UI review stage must be outputs/library-ui-review/<isolated-id>/site."
    );
  }
  return { normalizedStage, stageRoot };
}

export async function prepareLibraryUiReviewArtifact({
  root = process.cwd(),
  ...
  // Wrangler resolves Functions and configuration from --cwd. Remove the
  // entire deterministic container so ignored siblings from an older run
  // cannot survive beside site/ and alter this static-only deployment.
...
if (invokedAsScript) {
  const result = await prepareLibraryUiReviewArtifact({
    source: argumentValue("--source") || path.join(process.cwd(), "out"),
    stage: argumentValue("--stage")
  });
  console.log(
    `Prepared fail-closed UI review artifact: ${result.retainedFiles} files retained; `
      + `${result.markerFilesRemoved} preview-marker files removed.`
  );
}
