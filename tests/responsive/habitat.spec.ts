...
test('Desktop Hero fits the first screen and the new header remains keyboard operable', async ({ page }) => {
  ...
  await expect(page.locator('#technology-menu a')).toHaveCount(2);
  await expect(page.locator('#technology-menu a').nth(0)).toHaveText('COMPASS Interactive疑問が届く、参加型講義システム');
  await expect(page.locator('#technology-menu a').nth(0)).toHaveAttribute('href','INTRO_Interactive/');
  await expect(page.locator('#technology-menu a').nth(1)).toHaveText('COMPASS Platformテクノロジーを感じる3D体験');
  await expect(page.locator('#technology-menu a').nth(1)).toHaveAttribute('href','/3d/');
  const actions=page.locator('.site-header .header-actions a');
  await expect(actions).toHaveCount(2);
  await expect(actions.nth(0)).toHaveText('3Dを体験する');
  await expect(actions.nth(0)).toHaveAttribute('href','/3d/');
  await expect(actions.nth(1)).toHaveText('講義を体験する');
  await expect(actions.nth(1)).toHaveAttribute('href','INTRO_Interactive/');
...
test('Mobile preserves copy without fetching realtime habitat assets or engine', async ({ browser }) => {
  ...
  await expect(page.locator('#technology-menu')).not.toContainText('COMPASS Platform');
...
test('iPad landscape enters the immersive scene while portrait stays static', async ({ browser }) => {
  ...
  await expect(landscapePage.locator('.site-header .header-actions a').nth(0)).toHaveText('3Dを体験する');
  await expect(landscapePage.locator('.site-header .header-actions a').nth(1)).toHaveText('講義を体験する');
