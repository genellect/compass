...
Large HTML cards and text overlays have been removed. The room names are small physical signs in the render. Transparent native buttons use camera-projected door bounds; only hover/focus adds a restrained outline. The original destination question, accessible text alternatives, skip and playback status are outside the image. At small widths, controls maintain at least 44px tap targets. Form copy, fields, identity, header/footer and backend are preserved.
